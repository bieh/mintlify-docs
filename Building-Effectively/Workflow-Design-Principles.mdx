...
* Need to run the same logic for multiple inputs? Start with `Open a dataset`.
...
* **Use Small Data Sets for Testing:** When using `Open a dataset` or processing lists, test with a small number of items first to speed up debugging.
...
* **Variables:** Data from `Open a dataset` is injected into subsequent blocks.
...
* **Iteration:** When using `Open a dataset` or "Follow each link" in `Follow Links`, the subsequent blocks will run for each item/page, processing data in that specific context.
