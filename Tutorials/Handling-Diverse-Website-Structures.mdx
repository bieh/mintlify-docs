...
4.  Add a **`Find Links`** block. Configure it with a dynamic goal: `Find all links that must contain '{{contain}}'`. This uses the variable from our dataset.
5.  Add a **`Follow Links`** block next. Configure it to `Follow each link` found by the previous step.
6.  Add an **`Interact with Page`** block to click the spoiler. Configure it with the action: `Click on "Nutritional Information"`.
