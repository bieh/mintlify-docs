---
title: 'Block: Open Websites'
description: 'Define the starting URLs for your AI Agent to visit.'
icon: 'globe-network' # Example icon, replace with actual if available
---
...
1. **Adding URLs:**
   ...
   * **Single URL:** Type or paste a single URL into the input field and click **+ Add** (or press Enter).
   * **List of URLs:** You can often paste a list of URLs (e.g., copied from a spreadsheet, with each URL on a new line) directly into the input field. The system usually parses them into individual entries.
   * **Import from CSV:** Look for an option to import URLs from a CSV file. This is useful for large lists. The CSV typically needs a column containing the URLs.
...
2. **Managing URLs:**
   ...
   * **Toggle ON/OFF:** Each URL in the list usually has a toggle switch. Only URLs that are **ON** will be visited by the agent. This allows you to temporarily disable certain URLs without deleting them.
   * **Edit/Delete:** You can typically edit or delete individual URLs from the list.
...
3. **Using Variables (from `Apply Variables` block):**
   ...
   * **Full URL Replacement:** If a variable `{{website_url}}` contains a full URL (e.g., `https://jsonify.com`), you can simply put `{{website_url}}` in the URL input field.
...
* URL added: `https://jsonify.com`
* *Result:* The agent will open the Jsonify homepage.

**Example 2: Processing a List of Competitor Sites**
...
**Example 3: Scraping Paginated Search Results**

* Using "Add a numbered range of URLs":
...
* Assumes an `Apply Variables` block with a column `company_domain`.
* `Open Websites` block URL: `https://{{company_domain}}`
* *Result:* If `Apply Variables` has rows for `jsonify.com` and `google.com`, the agent will open these two sites.
