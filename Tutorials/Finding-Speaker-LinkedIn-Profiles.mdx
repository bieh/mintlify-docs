...
- `Open Websites`
- `Extract Data`
- `Open a dataset` (if starting with a predefined list of names not on a webpage)
- `Google Search`
- `Find Links`
- `Follow Links`
...
1. **`Start with Open a dataset Block:`**
   - Create a column named `full_name`.
   - Enter each person's name in a new row.
   - <Frame caption="Screenshot: Open a dataset block with a list of names">
       ![Screenshot: Open a dataset block with a list of names](/Tutorials/imgs/Tutorial-4-With-Varaibles.png)
     </Frame>
2. **`Proceed from Step 4 (Add and Configure Google Search Block)`** in Scenario A, using the `{{full_name}}` variable from the `Open a dataset` block. You would omit steps 2 and 3 from Scenario A.
