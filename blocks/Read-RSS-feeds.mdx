...
1. **Add a URL:**
   ...
   - You can also use variables from a preceding `Open a dataset` block to dynamically provide RSS feed URLs.
2. **Managing URLs:**
...
- **Data Source:** This block acts as a data source, similar to `Open a dataset`. The blocks that follow it will execute for each post fetched from the feed(s).
