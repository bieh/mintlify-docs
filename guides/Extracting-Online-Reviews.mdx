...
* Click the button sequence to create a new workflow: **Create an empty workflow ➙ Extract**.
    * This template will already include `Open Websites` and `Extract Data`. We will add `Paginate through Listings` manually.

### 2. Configure `Open Websites` Block

* Select the `Open Websites` block.
* We'll add a few Trustpilot company review pages as examples. In the URL input field, add the following URLs one by one (or use "Add a list of pages" if available):
...
### 3. Add and Configure `Paginate through Listings` Block

* After the `Open Websites` block, we need to tell the agent to go through multiple pages of reviews for *each* company.
* Click the **+** icon below the `Open Websites` block to add a new block.
* Search for and select the **`Paginate through Listings`** block (or a similarly named block for handling pagination).
* Select the `Paginate through Listings` block to configure it.
...
[Screenshot: Paginate through Listings block configured for 3 pages]

### 4. Configure `Extract Data` Block

* This block will run for each page loaded by the `Open Websites` and `Paginate through Listings` blocks.
* Select the `Extract Data` block (it should be after `Paginate through Listings`).
...
* Click the **Run manualy** button (usually at the top right).
* The agent will now:
    1.  Open the first company's Trustpilot page.
    2.  The `Paginate through Listings` block will take over, loading up to 3 pages of reviews for that company.
