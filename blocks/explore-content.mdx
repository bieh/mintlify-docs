...
### 2. Number of attempts to find the right page
...
### 3. How many pages (clicks) to use per attempt?

This defines the "budget" of clicks available to the agent for **each single attempt**. For example, a value of `2` allows the agent to make up to two clicks to find the page within one attempt.

<Warning>
The agent may use its entire click budget to go "deep" down a single path (e.g., click link A, then from page A click link B). It does not necessarily mean it will explore two separate links from the starting page. Plan your click budget accordingly. For exploring multiple distinct items from a list, other blocks might be more suitable.
</Warning>

\[Screenshot: Explore Content block configuration panel showing the new parameters]
...
2.  **Navigation:** It uses its goal description to decide which links to click. It will perform up to the number of clicks specified in its "click budget" for the current attempt.
...
* **Result:**
    * **Attempt 1:** The agent might click "Pricing" (1st click), see "Pro" and "Business" plans but not "Enterprise". It runs `Extract Data`, `plan_name` is "Business", `has_address` is "No". The goal is not met.
    * **Attempt 2:** Using the context that "Pricing" was a good first step, it navigates back to the pricing page (1st click), and now looks for a "Compare Plans" or "Contact Sales for Enterprise" link (2nd click). It lands on the Enterprise page. `Extract Data` runs, `plan_name` is "Enterprise". The goal is met, and the process stops.
...
* **Click Budget:** Use the click budget wisely. For finding a link that is likely two clicks away (e.g., in a dropdown menu), a budget of 2 is appropriate. For broader exploration, more attempts might be better than a large click budget.